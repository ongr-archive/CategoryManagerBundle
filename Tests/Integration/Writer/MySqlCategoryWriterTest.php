...
namespace Fox\CategoryManagerBundle\Tests\Integration\Writer;
...
class MySqlCategoryWriterTest extends BaseDatabaseTest
{
    ...
    public function testMySqlWriter($categories, $rootId, $result, $writerOptions)
    {
        ...
        $provider = new CategoryProvider('Fox\\CategoryManagerBundle\\Tests\\Integration\\Iterator\\DummyIterator');

        $container = $this->getContainer();

        /* @var TransferManager $manager */
        $manager = $container->get('fox_category_manager.transfer_manager');
        ...
        foreach ($categories as $category) {
            ...
            if ($category['parent']) {
                $entity->setParent($em->getReference('FoxCategoryManagerBundle:Category', $category['parent']));
            }
        ...
        $this->assertInstanceOf('Fox\\CategoryManagerBundle\\Entity\\Category', $rootNode);
